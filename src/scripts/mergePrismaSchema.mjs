...
async function mergePrismaSchemas() {
  try {
    ...
    for (const file of schemaFiles) {
      ...
      const commentedModelAdditions = content.match(/\/\*\*[\s\S]*?ADD TO (\w+) MODEL[\s\S]*?\*\/([\s\S]*?)(\*\/|$)/g);
...
function processModelAdditions(models, targetModelName, fieldLines) {
  ...
  console.warn(`⚠️ Warning: Target model ${targetModelName} not found for additions`);
}
  ...
  for (const modelDef of models) {
    ...
    if (modelNameMatch && modelNameMatch[1]) {
      ...
      if (!modelMap.has(modelName)) {
        ...
        console.log(`🔄 Found duplicate model: ${modelName}, merging fields...`);
        
        // Get existing model definition
        const existingModel = modelMap.get(modelName);
        
        // Extract fields from both models
        const existingFieldsMatch = existingModel.match(/model\s+\w+\s+{([\s\S]*)}/); 
        const newFieldsMatch = modelDef.match(/model\s+\w+\s+{([\s\S]*)}/); 
        
        if (existingFieldsMatch && existingFieldsMatch[1] && newFieldsMatch && newFieldsMatch[1]) {
          // Parse fields into individual lines for comparison
          const existingFieldLines = existingFieldsMatch[1].trim().split('\n')
            .map(line => line.trim())
            .filter(line => line.length > 0);
          
          const newFieldLines = newFieldsMatch[1].trim().split('\n')
            .map(line => line.trim())
            .filter(line => line.length > 0);
