...
function detectShadcnComponents(addonDir) {
  ...
  log(
    "🔍",
    `Found ${availableComponents.length} ShadCN components in project`,
    colors.blue
  );
  ...
  log("🟡", `Scanning JS/TS files: ${jsFiles.join(", ")}`, colors.yellow);
  if (jsFiles.length > 0) {
  ...
  for (const file of jsFiles) {
    ...
    while ((match = importStatementRegex.exec(content)) !== null) {
      ...
      log("🔵", `Found import in ${file}:`, colors.blue);
      log("🔵", `  Specifiers: ${importSpecifiers}`, colors.blue);
      log("🔵", `  Source: ${importSource}`, colors.blue);
      ...
      if (
        ...
        log("🟢", `  Matched UI import in ${file}`, colors.green);
...
function generateAddonConfig(addonName, addonDir) {
  ...
  if (allImports.size > 0) {
    ...
    if (filteredPackages.length > 0) {
      ...
      log(
        "💾",
        `Added ${filteredPackages.length} packages (filtered from ${allImports.size})`,
        colors.green
      );
    } else {
      log(
