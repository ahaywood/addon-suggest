...
function extractImports(filePath) {
  ...
  while ((match = importRegex.exec(content)) !== null) {
    ...
    if (!packageName.startsWith('.') && !packageName.startsWith('@redwoodsdk/')) {
...
function findRouteFile(addonDir) {
  const possibleRouteFiles = ['routes.ts', 'routes.tsx', 'routes.js', 'routes.jsx'];
...
function detectShadcnComponents(addonDir) {
  ...
  if (!fs.existsSync(projectUiDir)) {
    log('ℹ️', 'No ShadCN components directory found in project', colors.blue);
    return null;
  }
  
  // Get all ShadCN components in the project
  const availableComponents = fs.readdirSync(projectUiDir)
    .filter(file => file.endsWith('.tsx') || file.endsWith('.jsx'))
    .map(file => path.basename(file, path.extname(file)));
  
  if (availableComponents.length === 0) {
    log('ℹ️', 'No ShadCN components found in project', colors.blue);
    return null;
  }
  
  log('🔍', `Found ${availableComponents.length} ShadCN components in project`, colors.blue);
  ...
  for (const file of jsFiles) {
    ...
    for (const component of availableComponents) {
      // Check for import statements with this component
      const importRegex = new RegExp(`import\\s+\\{[^}]*\\b${component}\\b[^}]*\\}\\s+from\\s+['"]\\.\.?\\/components\\/ui['"]`);
      // Check for direct usage of the component in JSX
  ...
  if (usedComponents.size > 0) {
    log('✅', `Detected ${usedComponents.size} ShadCN components used in add-on`, colors.green);
    return [...usedComponents];
  }
  
  log('ℹ️', 'No ShadCN components used in add-on', colors.blue);
  return null;
}
...
function generateAddonConfig(addonName, addonDir) {
  ...
  if (allImports.size > 0) {
    ...
    const filteredPackages = filterPackages([...allImports], shadcnComponents && shadcnComponents.length > 0);
    
    if (filteredPackages.length > 0) {
      addonConfig.packages = filteredPackages;
      log('💾', `Added ${filteredPackages.length} packages (filtered from ${allImports.size})`, colors.green);
    } else {
      log('ℹ️', `No packages needed after filtering ${allImports.size} imports`, colors.blue);
    }
  }
